...
def set_speed(graph, n, v):
    V = [[0 for j in range(2*n*n)]for i in range(2*n*n)]
...
def set_distance(graph, n, d):
    D = [[0 for j in range(2*n*n)]for i in range(2*n*n)]
    for ind in graph.keys():
        for neigh in graph[ind]:
            D[ind][neigh] = d
    return D
...
def inst_to_file(seed):
    ...
    filename = "metroplex_"+str(seed)+".tsruami"
